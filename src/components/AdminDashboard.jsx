...
function StatusDot({ status }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:7 }}>
      ...
      <span style={{ fontSize:12, fontWeight:600, color: status==='offline' ? 'rgba(156,163,175,0.6)' : cfg.color }}>{cfg.label}</span>
    </div>
  );
}
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:14, marginBottom:20 }}>
            ...
            <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
              ...
              <button onClick={onRefresh} disabled={refreshing||loading}
                ...
                {refreshing ? <Loader2 style={{ width:14,height:14,animation:'spin 0.7s linear infinite' }}/> : <RefreshCcw style={{ width:14,height:14 }}/>}
                Refresh
              </button>
            </div>
          </div>
        ...
        <div style={{ borderRadius:16, overflow:'hidden', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(2,5,20,0.92)', boxShadow:'0 20px 60px rgba(0,0,0,0.5)' }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding:'14px 20px', fontSize:12, color:'rgba(140,180,255,0.55)', whiteSpace:'nowrap' }}>
                        {v.last_seen ? new Date(v.last_seen).toLocaleString('en-IN') : <span style={{ color:'rgba(100,140,200,0.3)' }}>Never</span>}
                      </td>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 20px', borderTop:'1px solid rgba(255,255,255,0.06)', background:'rgba(37,99,235,0.03)' }}>
            <span style={{ fontSize:12, color:'rgba(140,180,255,0.45)' }}>
              {filtered.length === 0 ? '0 vehicles' : `${(page-1)*PAGE_SIZE+1}–${Math.min(page*PAGE_SIZE,filtered.length)} of ${filtered.length} vehicles`}
            </span>
            {totalPages > 1 && (
              <div style={{ display:'flex', gap:6 }}>
                {[['‹ Prev',()=>setPage(p=>Math.max(1,p-1)),page===1],['Next ›',()=>setPage(p=>Math.min(totalPages,p+1)),page===totalPages]].map(([lbl,fn,dis])=>(
                  <button key={lbl} onClick={fn} disabled={dis}
                    style={{ padding:'6px 14px', borderRadius:8, border:'1px solid rgba(37,99,235,0.22)', background:'rgba(37,99,235,0.08)', color:'rgba(180,215,255,0.75)', fontSize:12, cursor:'pointer', opacity:dis?0.25:1, fontFamily:'inherit', transition:'all 0.15s' }}
                    onMouseEnter={e => { if(!dis) { e.currentTarget.style.background='rgba(37,99,235,0.18)'; e.currentTarget.style.color='rgba(220,240,255,0.9)'; }}}
