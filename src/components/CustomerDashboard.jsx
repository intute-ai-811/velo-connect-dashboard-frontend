// Fix #1: All imports moved to the top before any function definitions
...
export default function CustomerDashboard({ user, onLogout }) {
  ...
  // Fix #3: Wrapped in useCallback so useEffect dependency array is correct
  const load = useCallback(async () => {
    setError('');
    setLoading(true);
    console.log('[CustomerDashboard] load() called');
    try {
      const r = await api.get('/api/vehicles/my');
      console.log('[CustomerDashboard] API /my status=', r.status, 'data shape=', {
      ...
      console.log('[CustomerDashboard] setVehicles with', list.length, 'items');
      setVehicles(list);
    } catch (e) {
      console.error('[CustomerDashboard] load() error:', e.message, 'status=', e.response?.status, 'data=', e.response?.data);
  ...
  // Fix #4: Clean pagination buttons — no fragile tuple destructuring
  const paginationButtons = [
    { label: 'Previous', onClick: () => setPage(p => Math.max(1, p - 1)),           disabled: page === 1          },
    { label: 'Next',     onClick: () => setPage(p => Math.min(totalPages, p + 1)),  disabled: page === totalPages },
  ];

  // Fix #5: Safe row click with localStorage error handling
        ...
        <div style={{ borderRadius: 16, overflow: 'hidden', border: `1px solid ${T.border}`, background: 'rgba(2,5,20,0.8)' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${T.border}`, background: 'rgba(37,99,235,0.04)' }}>
                  {COLS.map(c => (
                    <th key={c} style={{ padding: '14px 20px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.14em', whiteSpace: 'nowrap' }}>
                      {c}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {loading && (vehicles ?? []).length === 0 ? (
                  <tr><td colSpan={COLS.length} style={{ padding: '80px 20px', textAlign: 'center' }}>
                    <Loader2 style={{ width: 28, height: 28, color: '#2563eb', animation: 'spin 0.7s linear infinite', margin: '0 auto 12px', display: 'block' }} />
                    <p style={{ color: T.textSub, fontSize: 13, margin: 0 }}>Loading your vehicles…</p>
                  </td></tr>
                ) : paginated.length === 0 ? (
                  <tr><td colSpan={COLS.length} style={{ padding: '80px 20px', textAlign: 'center', color: T.textMuted, fontSize: 13 }}>
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '14px 20px', fontSize: 11, color: T.textMuted }}>
                      ...
                      <td style={{ padding: '14px 20px', fontSize: 11, color: T.textMuted, whiteSpace: 'nowrap' }}>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderTop: `1px solid ${T.border}`, background: 'rgba(37,99,235,0.02)' }}>
              ...
              {/* Fix #4: Clean pagination — no fragile tuple destructuring */}
              <div style={{ display: 'flex', gap: 8 }}>
                {paginationButtons.map(({ label, onClick, disabled }) => (
                  <button
                    ...
                    onClick={onClick}
                    disabled={disabled}
                    style={{ padding: '6px 14px', borderRadius: 8, border: `1px solid ${T.border}`, background: 'rgba(37,99,235,0.06)', color: T.text, fontSize: 12, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.3 : 1, fontFamily: 'inherit', transition: 'all 0.15s' }}
                    onMouseEnter={e => { if (!disabled) e.currentTarget.style.background = 'rgba(37,99,235,0.14)'; }}
